 ...
 public:
  ...
  mapOptimization(const rclcpp::NodeOptions& options)
    ...
    pubPath = create_publisher<nav_msgs::msg::Path>(
        "liorf/mapping/path", QosPolicy(history_policy, reliability_policy));
    ...
    downSizeFilterLocalMapSurf.setLeafSize(surroundingKeyframeMapLeafSize,
                                           ...
                                           surroundingKeyframeMapLeafSize);
    downSizeFilterICP.setLeafSize(loopClosureICPSurfLeafSize,
  ...
  void gpsHandler(const sensor_msgs::msg::NavSatFix::SharedPtr gpsMsg) {
    if (gpsMsg->status.status != 0) return;
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  void publishGlobalMap() {
    ...
    for (int i = 0; i < (int)globalMapKeyPosesDS->size(); ++i) {
      if (common_lib_->pointDistance(globalMapKeyPosesDS->points[i],
  ...
  void performSCLoopClosure() {
    ...
    *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
    ...
    if (it != loopIndexContainer.end()) return;

    // std::cout << "SC loop found! between " << loopKeyCur << " and " <<
    // loopKeyPre << "." << std::endl; // giseop
        ...
        new pcl::PointCloud<PointType>());
    {
      ...
      loopFindNearKeyframes(cureKeyframeCloud, loopKeyCur, 0,
                            base_key);  // giseop
      loopFindNearKeyframes(prevKeyframeCloud, loopKeyPre,
                            historyKeyframeSearchNum, base_key);  // giseop
    ...
    // pcl::getTranslationAndEulerAngles (tCorrect, x, y, z, roll, pitch, yaw);
    // gtsam::Pose3 poseFrom = Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x,
    // y, z)); gtsam::Pose3 poseTo =
    // pclPointTogtsamPose3(copy_cloudKeyPoses6D->points[loopKeyPre]);

    // gtsam::Vector Vector6(6);
    // float noiseScore = icp.getFitnessScore();
    // Vector6 << noiseScore, noiseScore, noiseScore, noiseScore, noiseScore,
    // noiseScore; noiseModel::Diagonal::shared_ptr constraintNoise =
    // noiseModel::Diagonal::Variances(Vector6);

    // giseop
    pcl::getTranslationAndEulerAngles(correctionLidarFrame, x, y, z, roll,
                                      pitch, yaw);
    gtsam::Pose3 poseFrom =
        Pose3(Rot3::RzRyRx(roll, pitch, yaw), Point3(x, y, z));
    gtsam::Pose3 poseTo =
    ...
    robustConstraintNoise = gtsam::noiseModel::Robust::Create(
        gtsam::noiseModel::mEstimator::Cauchy::Create(
            1),  // optional: replacing Cauchy by DCS or GemanMcClure, but with
                 // a good front-end loop detector, Cauchy is empirically
                 // enough.
        gtsam::noiseModel::Diagonal::Variances(
            robustNoiseVector6));  // - checked it works. but with robust
                                   // kernel, map modification may be delayed
                                   // (i.e,. requires more true-positive loop
                                   // factors)
  ...
  void loopFindNearKeyframes(pcl::PointCloud<PointType>::Ptr& nearKeyframes,
    ...
    for (int i = -searchNum; i <= searchNum; ++i) {
      ...
      *nearKeyframes += *transformPointCloud(
          ...
          &copy_cloudKeyPoses6D->points[select_loop_index]);
    }
  ...
  void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract) {
    ...
    for (int i = 0; i < (int)cloudToExtract->size(); ++i) {
      if (common_lib_->pointDistance(cloudToExtract->points[i],
                                     cloudKeyPoses3D->back()) >
...
#pragma omp parallel for num_threads(numberOfCores)
  ...
  bool LMOptimization(int iterCount) {
    ...
    for (int i = 0; i < laserCloudSelNum; i++) {
      ...
      /*             float arx = (crx*sry*srz*pointOri.x +
         crx*crz*sry*pointOri.y - srx*sry*pointOri.z) * coeff.x
      ...
      float arx =
          (-srx * cry * pointOri.x -
           (srx * sry * srz + crx * crz) * pointOri.y +
  ...
  void scan2MapOptimization() {
    ...
    if (laserCloudSurfLastDSNum > 30) {
      ...
      RCLCPP_WARN(get_logger(),
                  "Not enough features! Only %d planar features available.",
  ...
  void addGPSFactor() {
    ...
    if (cloudKeyPoses3D->points.empty())
      ...
      if (common_lib_->pointDistance(cloudKeyPoses3D->front(),
    ...
    while (!gpsQueue.empty()) {
      if (ROS_TIME(gpsQueue.front().header.stamp) < timeLaserInfoCur - 0.2) {
        // message too old
        gpsQueue.pop_front();
      } else if (ROS_TIME(gpsQueue.front().header.stamp) >
  ...
  void publishFrames() {
    ...
    if (pubSLAMInfo->get_subscription_count() != 0) {
      ...
      //     pcl::PointCloud<PointType>::Ptr cloudOut(new
      //     pcl::PointCloud<PointType>()); *cloudOut += *laserCloudSurfLastDS;
      //     slamInfo.key_frame_cloud = publishCloud(rclcpp::Publisher(),
      //     cloudOut, timeLaserInfoStamp, lidarFrame); slamInfo.key_frame_poses
      //     = publishCloud(rclcpp::Publisher(), cloudKeyPoses6D,
      //     timeLaserInfoStamp, odometryFrame); pcl::PointCloud<PointType>::Ptr
      //     localMapOut(new pcl::PointCloud<PointType>()); *localMapOut +=
      //     *laserCloudSurfFromMapDS; slamInfo.key_frame_map =
      //     publishCloud(rclcpp::Publisher(), localMapOut, timeLaserInfoStamp,
      //     odometryFrame); pubSLAMInfo->publish(slamInfo); lastSLAMInfoPubSize
      //     = cloudKeyPoses6D->size();
      // }
    }
  }
