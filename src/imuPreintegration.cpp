 ...
 public:
  ...
  TransformFusion(const rclcpp::NodeOptions& options)
    ...
    subLaserOdometry = create_subscription<nav_msgs::msg::Odometry>(
        "liorf/mapping/odometry", QosPolicy(history_policy, reliability_policy),
        std::bind(&TransformFusion::lidarOdometryHandler, this,
                  std::placeholders::_1));
    ...
    subImuOdometry = create_subscription<nav_msgs::msg::Odometry>(
        ...
        std::bind(&TransformFusion::imuOdometryHandler, this,
                  std::placeholders::_1));
    ...
    pubImuPath = create_publisher<nav_msgs::msg::Path>(
        "liorf/imu/path", QosPolicy(history_policy, reliability_policy));
  }
  ...
  void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg) {
    ...
    // tf2::Transform map_to_odom = tf2::Transform(quat_tf, tf2::Vector3(0, 0,
    // 0));
    rclcpp::Time t(static_cast<uint32_t>(lidarOdomTime * 1e9));
    tf2::TimePoint time_point = tf2_ros::fromRclcpp(t);
    // tf2::Stamped<tf2::Transform> temp_map_to_odom(map_to_odom, time_point,
    // mapFrame); geometry_msgs::msg::TransformStamped trans_map_to_odom;
    ...
    if (lidarFrame != baselinkFrame) tCur *= lidar2Baselink;

    tf2::Stamped<tf2::Transform> temp_odom_to_base(tCur, time_point,
                                                   odometryFrame);
 ...
 public:
  ...
  IMUPreintegration(const rclcpp::NodeOptions& options)
    ...
    subOdometry = create_subscription<nav_msgs::msg::Odometry>(
        ...
        QosPolicy(history_policy, reliability_policy),
        std::bind(&IMUPreintegration::odometryHandler, this,
                  std::placeholders::_1));
  ...
  void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg) {
    ...
    if (systemInitialized == false) {
      ...
      while (!imuQueOpt.empty()) {
        if (ROS_TIME((&imuQueOpt.front())->header.stamp) <
    ...
    double lastImuQT = -1;
    while (!imuQueImu.empty() && ROS_TIME((&imuQueImu.front())->header.stamp) <
                                     currentCorrectionTime - delta_t) {
...
int main(int argc, char** argv) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
              "\033[1;32m----> IMU Preintegration Started.\033[0m");
