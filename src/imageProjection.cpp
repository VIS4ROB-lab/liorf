 ...
 public:
  ImageProjection(const rclcpp::NodeOptions &options)
    ...
    subLaserCloud = create_subscription<sensor_msgs::msg::PointCloud2>(
        pointCloudTopic, QosPolicy(history_policy, reliability_policy),
  ...
  void odomDeskewInfo() {
    ...
    while (!odomQueue.empty()) {
      if (ROS_TIME(odomQueue.front().header.stamp) <
          timeScanCur - sync_diff_time)
        odomQueue.pop_front();
      else
        break;
    }
    ...
    if (ROS_TIME(odomQueue.front().header.stamp) > timeScanCur) return;
    ...
    if (ROS_TIME(odomQueue.back().header.stamp) < timeScanEnd) return;
  ...
  void publishClouds() {
    ...
    cloudInfo.cloud_deskewed = publishCloud(pubExtractedCloud, fullCloud,
...
int main(int argc, char **argv) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"),
              "\033[1;32m----> Image Projection Started.\033[0m");
